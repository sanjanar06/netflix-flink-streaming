"""
Netflix Distributed Processing Dashboard
"""

from flask import Flask, render_template, jsonify
from kafka import KafkaConsumer
import json
import threading
import os
import time
from collections import defaultdict, deque

app = Flask(__name__)

# In-memory storage
metrics_queue = deque(maxlen=100)
active_sessions = defaultdict(dict)
session_history = deque(maxlen=50)


def consume_metrics():
    """Background Kafka consumer"""
    time.sleep(15)
    
    consumer = KafkaConsumer(
        'session-metrics',
        bootstrap_servers=os.getenv('KAFKA_BOOTSTRAP_SERVERS', 'kafka:29092'),
        value_deserializer=lambda m: json.loads(m.decode('utf-8')),
        group_id='visualization-dashboard',
        auto_offset_reset='latest'
    )

    for message in consumer:
        try:
            metric = message.value
            metrics_queue.append(metric)

            user_id = metric.get('userId')
            event_type = metric.get('eventType')
            task_manager = metric.get('taskManager', 'unknown')

            if event_type == 'SESSION_START':
                active_sessions[task_manager][user_id] = {
                    'userId': user_id,
                    'showName': metric.get('showName'),
                    'heartbeats': 0
                }

            elif event_type == 'HEARTBEAT':
                if task_manager in active_sessions and user_id in active_sessions[task_manager]:
                    active_sessions[task_manager][user_id]['heartbeats'] = metric.get('heartbeatCount', 0)

            elif event_type == 'SESSION_END':
                if task_manager in active_sessions and user_id in active_sessions[task_manager]:
                    session = active_sessions[task_manager].pop(user_id)
                    session['duration'] = metric.get('durationSeconds')
                    session['taskManager'] = task_manager
                    session_history.append(session)

        except Exception as e:
            print(f"Error: {e}")


threading.Thread(target=consume_metrics, daemon=True).start()


@app.route('/')
def index():
    return render_template('dashboard.html')


@app.route('/api/status')
def get_status():
    return jsonify({
        'taskmanagers': {
            tm: {'activeSessions': len(sessions)}
            for tm, sessions in active_sessions.items()
        },
        'activeSessions': {
            tm: list(sessions.values())
            for tm, sessions in active_sessions.items()
        },
        'recentMetrics': list(metrics_queue)[-10:],
        'sessionHistory': list(session_history)[-10:]
    })


if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5001, debug=True)
